    ...
    Statement = [
      {
        ...
        Effect   = "Allow"
        Resource = "arn:aws:iam::${var.namespace_to_aws_account_id["lab"]}:role/lab-${title(each.key)}-access-role"
      },
      {
        Action   = ["sts:AssumeRole"]
        Effect   = "Allow"
        Resource = "arn:aws:iam::${var.namespace_to_aws_account_id["prod"]}:role/prod-${title(each.key)}-access-role"
      }
    ]
  })
}
...
resource "aws_iam_policy" "child_account_access_policy" {
  for_each = local.root_enabled ? { for k, v in var.namespace_to_aws_account_id : k => v if k != "root" } : {}
